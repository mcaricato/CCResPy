...
def input_parser(input_file):
  ...
  for i in range(len(WLlist)): 
    ...
    Wlist.append(freq)
  #
  return ThrE, ThrA, MaxIt, Wlist, MaxD, RepD, PertType, tv, FreezeCore, memory, scratch, path_gauopen, eri, mol_inp, mol_out
...
########################################################
def getOvl(mol_inp,O,V,NB,ipbc,basis,dk,MOCoef):
  # basis: "AO" or "MO"
...
########################################################
def getFock(mol_inp,O,V,NB,ipbc,basis,dk,MOCoef):
  # basis: "AO" or "MO"
  # dk: = F: regular MO(k) basis, = T: dS/dK in MO(k) basis
  NOrb = O + V
...
def getPert(O, V, NB, ipbc, tv, MOCoef, Fock, pert_type, mol_inp, mol_out):
  ...
  if(ipbc):
    ...
    if(pert_type == "DipE" or pert_type == "FullOR_V" or pert_type == "FullOR_L"):
      ...
      #
      # dF/dk = F'
      FockDk = getFock(mol_inp,O,V,NB,ipbc,"MO",True,MOCoef)
      #
      # dS/dk = S'
      OvlDk = getOvl(mol_inp,O,V,NB,ipbc,"MO",True,MOCoef)
      #
